...
import { Download, Palette, Save, Edit3, Eye, FileText, Copy, CheckCircle, Sparkles } from 'lucide-react';
...
export const BusinessPlanStudio: React.FC<BusinessPlanStudioProps> = ({
  ...
  // Formulaire intégré (aligné sur tool.inputs)
  ...
  useEffect(() => {
    if (inputValue) {
      // For business plan tool, the main free-form field is usually "idea"
      setFormInputs((prev) => ({ ...prev, idea: inputValue }));
    }
  }, [inputValue]);

  const isHtmlDocument = useMemo(() => {
    const v = (editableContent || '').trim();
    if (!v) return false;
    return /<!doctype\s+html/i.test(v) || /<html[\s>]/i.test(v);
  }, [editableContent]);
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    // Validate required fields based on tool config when available
    ...
    if (onSubmit && tool?.promptGenerator) {
      const prompt = tool.promptGenerator(formInputs);
      if (prompt && !prompt.startsWith('ERROR:')) {
        onSubmit({ prompt, data: formInputs });
      }
    }
  };
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500">
      ...
      {!editableContent || editableContent.trim().length === 0 ? (
            ...
            {tool?.inputs?.map((input) => (
              <div key={input.name}>
                ...
                {input.type === 'richtext' || input.useEditor ? (
                  <div className="bg-white/10 border-2 border-white/30 rounded-lg overflow-hidden">
                    <RichTextEditor
                      content={formInputs[input.name] || ''}
                      onChange={(content) => setFormInputs((prev) => ({ ...prev, [input.name]: content }))}
                ...
                {input.helpText && <p className="text-xs text-white/70 mt-2 italic">{input.helpText}</p>}
              </div>
            ))}
        ...
        <>
          {/* If the output is a full HTML document, render it as HTML for a reliable preview */}
          {isHtmlDocument ? (
            <HtmlResultDisplay
