export const handleLocalTool = (command: string): string => {
  ...
  switch (action) {
    ...
    case 'JSON_FORMAT':
      try {
        ...
        return "❌ **Erreur JSON** : Le format est invalide.\nVérifiez les virgules et les guillemets.";
      }
    ...
    case 'DECISION_MAKER':
        ...
        if (choices.length < 2) return "❌ Il faut au moins 2 choix séparés par des virgules.";
        const winner = choices[Math.floor(Math.random() * choices.length)];
        return `### 🎲 Le sort a décidé :\n\n# **${winner}**`;
    ...
    case 'SLUG_GEN':
        ...
        return `### Slug URL optimisé :\n\n\`${slug}\``;

    case 'PX_TO_REM':
        const px = parseFloat(input);
        if (isNaN(px)) return "❌ Entrez un nombre valide.";
        const rem = px / 16;
    ...
    case 'CSV_JSON':
        try {
            ...
            if (lines.length < 2) return "❌ Il faut au moins une ligne d'en-tête et une ligne de données.";
            ...
            return "❌ Format CSV invalide. Assurez-vous d'utiliser des virgules.";
        }

    case 'HEX_RGB':
        const hex = input.replace('#', '');
        if (hex.length !== 6) return "❌ Code Hex invalide (doit être 6 caractères, ex: FFFFFF)";
        const r = parseInt(hex.substring(0, 2), 16);
        const g = parseInt(hex.substring(2, 4), 16);
        const b = parseInt(hex.substring(4, 6), 16);
        return `### Conversion Couleur 🎨\n\n**HEX :** #${hex.toUpperCase()}\n**RGB :** rgb(${r}, ${g}, ${b})\n\n<div style="width:100%; height:50px; background-color:#${hex}; border:2px solid #000; border-radius:8px;"></div>`;
    ...
    case 'PERCENT_CALC':
        ...
        if(isNaN(val) || isNaN(total)) return "❌ Valeurs invalides";
...
\`\`\``;
    ...
    default:
      return "Commande locale inconnue.";
  }
};